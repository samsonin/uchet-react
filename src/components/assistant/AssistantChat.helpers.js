...
export const getDailyAssistantIntro = (assistantName, app = {}) => {
    ...
    return (
        `Вы открыли раздел «Ежедневный отчет». ${stockText} выбрана, смена уже открыта. ` +
        "Проверьте сотрудников смены, предоплаты для предзаказов товара или услуги, товары, работы и услуги, расходы, зарплату, подотчеты и способы оплаты. " +
        "Внизу проверьте итоговые суммы: остаток на утро, выручку, подотчеты, безналичные оплаты, сданную сумму и остаток. " +
        "Если работа закончена, используйте кнопку «Выйти» в меню пользователя."
    );
};
