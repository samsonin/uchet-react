...
export default function GoodPictureCapturePage() {
    const { token } = useParams();
    const [status, setStatus] = useState("idle");
    const [error, setError] = useState("");

    const uploadPath = useMemo(() => (
        `${GOOD_PICTURE_SESSION_PATH}/${encodeURIComponent(token || "")}/image`
    ), [token]);
    ...
    return <main className="good-picture-capture-page">
        <section className="good-picture-capture-panel">
            <Typography variant="h5" component="h1">
                Фото товара
            </Typography>

            {status === "done"
                ? <Typography className="good-picture-capture-message">
                    Фото принято. Можно вернуться к компьютеру.
                </Typography>
                : <>
                    <Typography className="good-picture-capture-message">
                        Сделайте фото товара или выберите готовое изображение.
                    </Typography>
                    <Button
                        component="label"
                        variant="contained"
            ...
            {error && <Typography className="good-picture-capture-error">
                {error}
            </Typography>}
        </section>
    </main>;
